package com.github.pauloo27.discord.bot.utils;
...
public class SimpleLogger {
    ...
    static {
        if (Bot.getInstance().getConfig() != null)
            formatter = new SimpleDateFormat(Bot.getInstance().getConfig().getDateFormat());
        else
    ...
    public static void sendLogToOwner(String message) {
        val pv = Bot.getInstance().getJda().getUserById(Bot.getInstance().getConfig().getOwnerId()).openPrivateChannel().complete();
